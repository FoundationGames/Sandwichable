...
public class CuttingBoardBlockEntityRenderer implements BlockEntityRenderer<CuttingBoardBlockEntity> {
    ...
    public void render(CuttingBoardBlockEntity blockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        for (int i = 0; i < stack.getCount(); i++) {
            ...
            MinecraftClient.getInstance().getItemRenderer().renderItem(stack, ModelTransformation.Mode.GROUND, light, overlay, matrices, vertexConsumers, i + 81649763);
            matrices.pop();
        }
        ...
        MinecraftClient.getInstance().getItemRenderer().renderItem(blockEntity.getKnife(), ModelTransformation.Mode.GROUND, light, overlay, matrices, vertexConsumers, 1894787523);
        matrices.pop();
    }
}
